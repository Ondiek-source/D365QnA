...
const QuizEngine = (function () {
    ...
    /**
     * Get DOM elements — always fresh, never cached.
     * Elements live inside hidden screens at init time so a
     * one-time cache returns null for all of them.
     */
    function _getDOM() {
        _dom = {
    ...
    return {
        ...
        launchFreshTopic: async function (topicId) {
            ...
            try {
                ...
                // Switch to quiz screen then render first question
                if (window.UI) window.UI.showScreen('quizScreen');
                this.renderQuestion();
        ...
        renderQuestion: function () {
            if (!_state || !_state.questions) return;
            _getDOM(); // always refresh — elements were hidden at init time
            ...
            const qType = q.type || q._type;

            if (_dom.qTypeBadge) {
                _dom.qTypeBadge.textContent = typeLabelMap[qType] || qType;
                _dom.qTypeBadge.className = `q-type-badge ${typeClsMap[qType] || 'qtb-mcq'}`;
            }
            ...
            // Dispatch to specific renderers — _type is set by buildSession, type by raw JSON
        ...
        renderDragDrop: function (q) {
            ...
            _dom.questionBlock.innerHTML = `
                <div class="q-block">
                    ...
                    <div class="dd-layout" style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
                        <div>
                        ...
                        <div>
                            <div class="dd-pool-label">Drop zones</div>
        ...
        renderScenarioPanel: function (q) {
            ...
            const scenarioTitle = q._scenarioTitle || q.scenarioTitle || 'Scenario';
            const scenarioBody  = q._scenarioBody  || q.scenarioBody  || q.scenarioText || '';
            const siblings = _state.questions.filter(x =>
                (x._type === 'casestudy' || x.type === 'casestudy') &&
                (x._scenarioTitle || x.scenarioTitle) === scenarioTitle
            );
            const subListHtml = siblings.map((sq, si) => {
                ...
                const sqText = (sq.q || '').substring(0, 60) + ((sq.q || '').length > 60 ? '…' : '');
            ...
            _dom.scenarioBody.innerHTML = `
                <span class="scenario-label">${scenarioTitle}</span>
                <div style="font-size:12px;line-height:1.75;color:var(--muted2)">${scenarioBody}</div>
                <div class="scenario-q-list">
                    ...
                    ${subListHtml || '<div style="font-size:11px;color:var(--muted)">No sub-questions found</div>'}
                </div>`;
        },
        ...
        showAnsweredState: function (q, ans) {
            const qType = q.type || q._type;
            if ((qType === 'mcq' || qType === 'tf') && ans.selected !== undefined) {
                const correctIdx = q.shuffledAnswer !== undefined ? q.shuffledAnswer : q.answer;
                const btns = document.querySelectorAll('.opt-btn');
                btns.forEach(b => b.disabled = true);
                btns.forEach((b, i) => {
            ...
            const record = {
                ...
                answers: _state.answers.map((a, i) => {
                    ...
                    return {
                        ...
                        msRef: q.msRef || null,
                        correctAnswer: (q.type || q._type) !== 'dragdrop' ? ((q.shuffledOptions || q.options)?.[(q.shuffledAnswer !== undefined ? q.shuffledAnswer : q.answer)] || '') : 'See mapping',
